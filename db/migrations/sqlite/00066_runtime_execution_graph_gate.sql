...
ALTER TABLE runtime_graph_edge_runs RENAME TO runtime_graph_edge_runs_old;
ALTER TABLE runtime_graph_node_runs RENAME TO runtime_graph_node_runs_old;
...
SELECT
    ...
    runtime_round_id, agent_round_id, agent_id, name, description,
    status, failed, started_at, updated_at, finished_at, metadata_json
FROM runtime_graph_node_runs_old;
...
SELECT
    edge_run_id, graph_id, owner_user_id, session_key,
    source_node_run_id, target_node_run_id, edge_kind, created_at
FROM runtime_graph_edge_runs_old;

DROP TABLE runtime_graph_edge_runs_old;
DROP TABLE runtime_graph_node_runs_old;
...
ALTER TABLE runtime_graph_edge_runs RENAME TO runtime_graph_edge_runs_expanded;
ALTER TABLE runtime_graph_node_runs RENAME TO runtime_graph_node_runs_expanded;
...
SELECT
    ...
    runtime_round_id, agent_round_id, agent_id, name, description,
    status, failed, started_at, updated_at, finished_at, metadata_json
FROM runtime_graph_node_runs_expanded
WHERE node_kind IN ('agent', 'subagent', 'tool');
...
FROM runtime_graph_edge_runs_expanded AS edge
WHERE edge.edge_kind IN ('invoke', 'spawn')
  AND EXISTS (
      SELECT 1 FROM runtime_graph_node_runs AS source_node
      WHERE source_node.node_run_id = edge.source_node_run_id
  )
  AND EXISTS (
      SELECT 1 FROM runtime_graph_node_runs AS target_node
      WHERE target_node.node_run_id = edge.target_node_run_id
  );

DROP TABLE runtime_graph_edge_runs_expanded;
DROP TABLE runtime_graph_node_runs_expanded;
