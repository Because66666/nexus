...
ALTER TABLE connector_oauth_states ADD COLUMN control_flow_id TEXT;
CREATE INDEX idx_connector_oauth_states_control_flow
...
CREATE TABLE connector_authorization_flows (
    ...
    start_configuration_version INTEGER NOT NULL,
    expected_configuration_version INTEGER NOT NULL,
    completed_configuration_version INTEGER,
    ...
    human_approved_at DATETIME NOT NULL,
    opened_at DATETIME,
    next_poll_at DATETIME,
    poll_claim_until DATETIME,
...
CREATE INDEX idx_connector_authorization_flows_owner_session
    ON connector_authorization_flows (
        owner_user_id, agent_id, business_session_key, created_at
    );
CREATE INDEX idx_connector_authorization_flows_owner_connector
    ON connector_authorization_flows (
        owner_user_id, connector_id, status, created_at
    );
CREATE INDEX idx_connector_authorization_flows_expiry
...
ALTER TABLE connector_oauth_states DROP COLUMN control_flow_id;
