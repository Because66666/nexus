...
- Removed the empty desktop-only `/app` header so Home starts at the top of the Web viewport; restored the Windows minimize, maximize/restore, close, four-edge resize, native-titlebar drag, double-click, and system-menu interactions; and replaced child-HWND clipping with the supported WPF composition control beneath a self-contained native title row.
