...
- Unified imported third-party Skills behind `<config>/users/<owner>/.agents/skills`, a global owner-scoped source shared by nxs and Claude Code; Agent records now persist `external:<skill_name>` references, and legacy workspace copies are migrated and removed.
