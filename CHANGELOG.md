...
- Replaced WorkGraph Bézier fans with orthogonal polylines, strengthened neutral forward edges, and muted loop-back saturation; return routing now treats node intersection as the hard conflict while allowing same-side returns to merge after their short source stems onto one shared U-shaped bus and arrow port inside the owning subgraph, reducing visual line count without covering nodes. Sibling retries share lower flow rails, while Subagent ownership stays inside one primary runtime frame without nested containers.
