...
- Aligned responsive Web Headers with each platform's window chrome: macOS keeps native traffic lights in the shared Header plane and retains threshold-based click/drag arbitration, while Windows now uses a dedicated 34-DIP native title/menu row above the untouched Web layout, with an app icon, back/forward navigation, working File/Edit/View/Help menus, and caption controls. Windows Header backgrounds use WebView2's native non-client hit testing while tabs, buttons, links, menus, and editable controls preserve normal Web interaction. Web centers its collapsed control on the 48px Dock, removes the empty `/app` drag Header and all Windows caption-control reservation, and keeps Launcher branding constrained to its dedicated visible Header instead of turning the content canvas into a drag surface.
...
- Removed the empty desktop-only `/app` header so Home starts at the top of the Web viewport; restored the Windows minimize, maximize/restore, close, four-edge resize, native/Web drag, double-click, and system-menu interactions; and replaced child-HWND clipping with the supported WPF composition control beneath a self-contained native title row.
