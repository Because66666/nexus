...
- Aligned responsive Web Headers with each platform's window chrome: macOS keeps native traffic lights in the shared Header plane and retains threshold-based click/drag arbitration, while Windows now uses a dedicated 34-DIP native title/menu row above the untouched Web layout, with an app icon, back/forward navigation, working File/Edit/View/Help menus, caption controls, and the only Windows drag surface. The Windows WebView remains entirely in the client area so tabs, buttons, links, menus, editable controls, and Header whitespace preserve normal Web interaction. Web centers its collapsed control on the 48px Dock, keeps the macOS-only `/app` transparent drag surface out of browser and Windows layouts, and keeps Launcher branding constrained to its dedicated visible Header instead of turning the content canvas into a drag surface.
- Restyled the Windows title-bar menus around the Nexus overlay recipe: a high-opacity neutral surface, 16px shell and 8px item radii, warm neutral states, aligned shortcut columns, inset separators, and native keyboard navigation intact.
- Unified Windows menu feedback, update prompts, and startup errors behind one token-backed Nexus native dialog with shared typography, controls, details rendering, focus behavior, and action hierarchy, replacing the system message boxes and standalone update window.
...
- Limited Windows dragging to the dedicated 34-DIP native app bar, leaving every Web Header and content surface in the normal client area, and removed the extra shadow layer around native menu popups.
- Removed the empty `/app` header from browser and Windows layouts while retaining a transparent macOS-only 60px main-content drag surface; restored the Windows minimize, maximize/restore, close, four-edge resize, native-titlebar drag, double-click, and system-menu interactions; and replaced child-HWND clipping with the supported WPF composition control beneath a self-contained native title row.
- Fixed overflowing Session tabs in WebView2 by explicitly mapping both mouse-wheel and touchpad `deltaX`/`deltaY` input onto the horizontal viewport with a non-passive listener, while allowing page scrolling to continue at the strip boundaries.
