...
- Removed the empty desktop-only `/app` header so the Home canvas starts at the top on macOS and Windows; restored the Windows minimize, maximize/restore, close, four-edge resize, drag, double-click, and system-menu interactions; and replaced child-HWND clipping with the supported WPF composition control so transparent caption buttons inherit the real Web theme surface without a mismatched native color block.
