...
- Aligned responsive Web Headers with each platform's window chrome: macOS keeps native traffic lights in the shared Header plane, while Windows now uses a dedicated 34-DIP native title/menu row above the untouched Web layout, with an app icon, back/forward navigation, working File/Edit/View/Help menus, and caption controls. Both hosts arbitrate clicks versus native window drag across tabs, buttons, links, menus, and Header backgrounds with a 4-DIP threshold while preserving editable controls. Web centers its collapsed control on the 48px Dock, removes the empty `/app` drag Header and all Windows caption-control reservation, and keeps Launcher branding constrained to its dedicated visible Header instead of turning the content canvas into a drag surface.
