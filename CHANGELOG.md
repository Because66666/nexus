...
- Added one authoritative DM/Room Execution Graph resource with two coordinated views: a compact live avatar rail above the Composer and an icon-first Header surface for inspecting the complete graph. Work Item responsibilities render as primary Agent nodes, exact child Attempts render as nested Subagent nodes, and typed dependency/spawn edges carry direction without edge prose. Historical child runs remain detail-only, Work Item containment no longer changes readiness/layout depth, and exact Agent-round Task steps stay inside the owning node detail.
- Simplified the collapsed Execution Graph control to its avatar node rail, moved current work and progress into the expanded header, and taught the shared execution prompt to keep lifecycle narration in Graph events while Agent replies prioritize actual task results.
...
- Changed `plan_execution` to accept exactly one portable WorkGraph transport: a native `items` object array expressed with the common function-calling JSON Schema subset. This removes fragile JSON-inside-JSON escaping and model-specific capability gates while preserving typed decoding, domain validation, and atomic Plan persistence across Providers.
