...
- Fixed owner transcript reads failing after nxs recreated JSONL files with a 0600 ACL mask; the trusted launcher now repairs the owner runtime ACL and retries the confined read, while shell command substitution and literal regular-expression syntax such as `$(whoami)` and `^[0-9]+$` no longer produce false path escape denials.
