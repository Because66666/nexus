...
- Added a backend-owned Slash catalog for the shared DM/Room Composer: Nexus ships versioned nxs and Claude command manifests, merges the selected runtime manifest with Nexus host commands after session bind, and lets `/` show a compact, bounded-height input-width command menu with internal list scrolling before subsequent text filters it, without starting or probing a runtime. `/model` now opens the runtime-aware model picker, while `/skills` opens the complete Skill picker and permits explicit insertion even when a Skill is not enabled for the current Agent.
