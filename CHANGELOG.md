...
- Smoothed DM, Room, and Thread output tracking with one retargetable spring, kept wheel, pointer, touch, and keyboard upward gestures detached from live output, inherited the existing offset when a static Feed becomes virtual, preserved the same visible round across renderer and node-topology changes, and stopped active following before Composer or App/browser viewport height changes can rewrite the reading position, including reduced-motion mode. The return-to-latest control now occupies a fixed safe band outside the Feed so showing it never covers content or resizes the reading viewport.
- Kept each Room Agent reply in one stable slot from pending through completion, treated permissions and terminal component replacement as atomic layout changes, anchored the first visible round while earlier permissions, terminal results, or new member replies change height, prevented detached reading from being recaptured or live bottom-following from reversing during transient measurement, and preserved optimistic user nodes through acknowledgement.
...
- Fixed owner transcript reads failing after nxs recreated JSONL files with a 0600 ACL mask; the trusted launcher now repairs the owner runtime ACL and retries the confined read, while shell command substitution syntax such as `$(whoami)` no longer produces a false path escape denial.
