...
- `agent-message-event-handlers.ts` 处理消息快照与流式载荷；完整 snapshot 写入前同步 flush 已排队 stream patch，保持同一 WebSocket 的到达顺序，并在 snapshot 提交后通知 transport 于下一帧清理 live reveal 标记。
