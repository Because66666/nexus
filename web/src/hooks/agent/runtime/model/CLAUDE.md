...
- 消息终态迁移统一解析为保留、移除或更新状态三种动作，调用方只定义作用域规则；round 终态移除 ephemeral 过程消息，但保留已经完成的 transient host 通知。
