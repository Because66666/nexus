...
- `command_catalog` 只作为 runtime prompt 补全数据返回；会话连接成功以及 runtime 一轮结束回到 idle 时只读刷新，避免保留初始化前的空快照；用户真正打开 Slash 浮层时才允许按需连接尚未启动的 DM runtime。宿主控制动作继续走既有 typed action，不在 Composer 内直接解释 runtime 内部元数据。
