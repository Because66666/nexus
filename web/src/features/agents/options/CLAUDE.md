...
- Agent 选项块采用可触控但克制的控件高度和明确的组间距；内联详情头像保持 56px 识别尺度，左缘与正文 gutter 对齐，并通过中性文字动作打开网格选择面板，不得把头像或蓝色 pill 做成页面主视觉。工具页把权限模式投影为自适应紧凑选择网格，把预授权工具投影为桌面三列、窄窗逐级收拢的单行摘要卡；选中态复用共享中性底色，安全说明保持可见但不再占用独立警告卡。
