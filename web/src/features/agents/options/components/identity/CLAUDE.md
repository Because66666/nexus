...
- `identity-layout.ts` 用完整描述表维护 dialog/inline 布局差异；dialog 在 `md` 以下堆叠资料、标签与模型字段，并维持可触控的字段高度和分组间距，覆盖手机和窄平板而不复制整套 JSX；inline 窄屏列必须由内容自然撑高，桌面用“资料 + 标签 / 横跨整行的模型”两行网格平铺内容，避免字段组之间出现无语义空白带。
