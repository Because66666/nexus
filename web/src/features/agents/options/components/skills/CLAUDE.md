...
- 技能选项卡使用紧凑但可自然增高的卡片，并复用管理目录的三列、两列、单列响应式网格；卡片使用 `skill.name` 生成跨页面稳定的静态数学曲线身份标记，标题、徽标和两行说明不得被固定高度挤压。分组与开关已经表达启用状态，行内不再重复“启用/已启用”文字；系统内置状态由标题旁锁定徽标表达，右侧不再重复状态。
