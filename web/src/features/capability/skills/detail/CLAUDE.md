...
- `skill-detail-model.ts` 用标签化快照表达加载、失败和就绪状态，集中来源、图标、徽标、链接投影和 Markdown 前导内容转换；内置 Skill 只接收共享展示层给出的本地化说明，不改写详情资源。
- `skill-detail-view.tsx` 只渲染模型、Agent 使用矩阵和触发命令，不直接调用 API、维护 Effect 或复制市场反馈。
- 窄屏返回语义由应用页面 Header 提供，详情内部面包屑只在桌面显示；正文保持受控阅读宽度。
