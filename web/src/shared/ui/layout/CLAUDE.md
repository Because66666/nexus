...
- `workspace-content-header.tsx` 是管理页正文标题、单句说明与页面动作的唯一 Header；共享顶部补偿必须让 Web 标题对齐侧栏品牌栏、让 macOS 标题对齐原生控件。在 macOS 统一标题栏中还必须参与窗口手势，并在侧栏折叠或无侧栏时通过共享横向安全区避开 traffic lights 与侧栏展开入口，不得给具体页面添加私有 padding。
