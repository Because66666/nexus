...
- `workspace-content-header.tsx` 是管理页正文标题、单句说明与页面动作的唯一 Header；桌面态的标题、说明和动作必须压进共享顶栏，浏览器使用 60px，macOS 由原生红灯 Y 中心推导高度，底部分隔线与侧栏品牌栏贯通。共享顶部补偿必须让 Web 标题对齐侧栏品牌栏、让 macOS 标题避开原生控件；统一标题栏还必须参与窗口手势，并在侧栏折叠或无侧栏时通过共享横向安全区避开 traffic lights 与侧栏展开入口，不得给具体页面添加私有 padding。
