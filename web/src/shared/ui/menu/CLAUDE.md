...
菜单组件直接导入，不通过另一个菜单文件隐式转出。锚点定位、材质与浏览器生命周期统一复用 `shared/ui/overlay/`；业务需要扩大弹层时传 `menuMinWidth`，不要用菜单样式类覆盖定位计算。单行触发器保留完整活动标签的原生 `title` 兜底，并独立声明正常行高，避免继承紧凑按钮的 `leading-none` 后裁掉英文下行字形。
