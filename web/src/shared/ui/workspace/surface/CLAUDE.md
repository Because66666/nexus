...
- 工作区视图标签遵循 Session 标签的状态层级：未选中透明、悬停轻微提亮，活动项使用中性底色与文字权重区分，不恢复描边或阴影；没有 Session 导航的目录页改用底边指示当前视图，避免把页面级 Tab 做成按钮组。View Tab、历史、成员与溢出入口统一使用 8px 圆角矩形，只有状态点和真实徽标保留全圆。只有当前可关闭的视图显示 `×`，但同组项目始终预留等宽关闭槽位，关闭按钮必须位于活动底面之上的独立点击层，切换不得改变标签宽度。
