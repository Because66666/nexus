...
- 默认打开集合保留全部最近会话，不得设置与实际可用空间无关的标签数量上限。标签共同占满可用轨道，每新增一个会话都按活动态权重平滑缩窄；达到可读最小宽度后，不再硬挤或移除旧标签，而是进入可由触控板、鼠标拖拽与滚动轨道直接浏览的横向溢出区。视觉遵循 Safari 标签栏：当前标签使用带轻阴影的浮起圆角底面，非当前标签保持透明并以细分隔线分段，关闭动作只在悬停或键盘聚焦时出现。创建入口固定在轨道右端，以无文字 `+` 图标呈现；会话总览只在标签真实溢出时出现，并作为同一椭圆胶囊的左分区与创建入口组合。滚动轨道同样只在真实溢出时出现。总览列出所有最近会话，选择被收纳的会话时须先把它加入打开集合。会话正文由上层仅挂载当前活动会话。
