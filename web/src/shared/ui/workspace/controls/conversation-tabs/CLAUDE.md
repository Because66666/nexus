...
- `conversation-tabs-model.ts` 定义创建时间排序、打开集合、活动标签、关闭邻居、稳定溢出判定和宽度分配等纯模型；标签按 `created_at` 从旧到新稳定排列，激活或消息活动不得重排，打开集合按保留存活项、追加外部选中项、保证非空三阶段归约。溢出判定只依赖轨道宽度、固定边缘动作与标签最小可读宽度，不得读取宽度动画中的瞬时 DOM 溢出状态。
...
- 默认打开集合保留全部会话，不得设置与实际可用空间无关的标签数量上限。标签共同占满可用轨道，每新增一个会话都按活动态权重平滑缩窄；达到可读最小宽度后，不再硬挤或移除旧标签，而是进入可由触控板、鼠标拖拽与滚动轨道直接浏览的横向溢出区。溢出宽度以活动标签为锚重新计算一屏可完整容纳的普通标签数量，滚动结束时吸附到标签边界，尽量避免可视区停在半个标签上。视觉遵循无阴影的轻量导航层级：当前标签用中性浅底、状态点和中等字重与工作面区分，非当前标签透明，只有连续非活动标签之间保留短竖线，关闭动作只在悬停或键盘聚焦时出现。历史入口固定在导航带左端并承载选择、重命名、删除与分页的唯一会话清单；创建入口固定在右端，以无文字 `+` 图标呈现。滚动轨道只在真实溢出时出现。会话正文由上层仅挂载当前活动会话。
