...
- 技能市场由 `features/capability/skills/controller/` 按目录、外部搜索、来源和操作拆分状态；Nexus 内置 Skill 的双语说明统一由 `lib/skill-description.ts` 做只读展示投影，并由目录、详情、Agent、Room 与 Composer 复用；Skill 卡片共用 `shared/ui/display/seeded-avatar.tsx` 按稳定名称生成跨页面一致的资源身份；子视图只消费窄 Props，不得依赖完整控制器
