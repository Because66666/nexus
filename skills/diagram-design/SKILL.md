...
tags:
  ...
  - visual-explanation
description: >-
  把架构、流程、时序、状态、数据模型、时间线、泳道、象限、组织关系、层级、集合、漏斗、
  循环、甘特和数据平台关系制作成专业的单文件 HTML + SVG 图解。用户要求画流程图、
  系统架构图、关系图、概念图、技术示意图，或希望用视觉解释复杂结构时使用。支持 27 种
  图解类型、品牌配色、本地模板、编辑式标注、浅色/深色/长文/终端变体；不用于完整 PPT、
  纯数值统计图表、地图或一句话即可表达的简单内容。
recommendation: 适合把流程、架构、关系与抽象概念制作成清晰、可交付的专业图解。
allowed-tools:
  - Read
  - Write
  - Bash
...
> **来源**：本内置 Skill 基于 Cathryn Lavery 的
> [`diagram-design`](https://github.com/cathrynlavery/diagram-design) 修改并适配 Nexus，固定来源提交为
> [`a157f7616473d966d6f433cf0b4d4f1880603504`](https://github.com/cathrynlavery/diagram-design/commit/a157f7616473d966d6f433cf0b4d4f1880603504)。
> 上游采用 MIT 许可证；完整来源、修改范围和第三方说明见 [SOURCE.md](SOURCE.md)，许可全文见
> [LICENSE](LICENSE)，图标与第三方素材归属见 [THIRD_PARTY_LICENSES.md](THIRD_PARTY_LICENSES.md)。

---

## 0. First-time setup — style guide gate

**Before generating your first diagram in a new project, check for a project-local style profile at
`.diagram-design/style-guide.md`. Never edit the built-in `references/style-guide.md`; platform Skills
are shared, immutable references.**

If the project-local profile is missing, **pause and ask the user**:

> *"This is the first diagram in this project. Do you want to customize it to match your brand first?
> Options: (a) pull from a website URL, (b) extract from an installed Skill, (c) extract from a local
> design-system folder, (d) paste tokens manually, or (e) use the neutral default."*

Then branch:

- **(a)** → follow [`references/onboarding.md § URL`](references/onboarding.md) to fetch the site,
  extract palette + fonts, propose a diff, and write `.diagram-design/style-guide.md`.
...
- **(d)** → accept the user's tokens and write them to `.diagram-design/style-guide.md`.
- **(e)** → copy the built-in `references/style-guide.md` to `.diagram-design/style-guide.md`, preserving
  the default as an explicit project choice.

Once the local profile exists, skip this gate on subsequent runs. Resolve tokens from the project-local
profile first, then fall back to the built-in reference only when no project context exists.
...
**The design system is skinnable.** The built-in
[`references/style-guide.md`](references/style-guide.md) defines the neutral defaults. A project-local
`.diagram-design/style-guide.md` overrides those semantic roles (`paper`, `ink`, `muted`, `accent`,
`link`, …). Never modify the built-in file; use the onboarding flow in
[`references/onboarding.md`](references/onboarding.md) to create or update the project-local profile.

> When specs below or in type references mention "ink", "accent", "muted", etc., look up the value in
> `.diagram-design/style-guide.md` first, then fall back to `references/style-guide.md`.
...
- Icon set (laptop, server, DB, K8s, Docker, AWS, …) →
  [primitive-icons.md](references/primitive-icons.md). Browse the complete upstream gallery at
  [assets/icons.html](assets/icons.html), or reuse the 86 original SVG files under
  [assets/icons/](assets/icons/README.md); keep each icon's recorded provenance and license warning.
...
1. Copy the variant closest to what you want from `${CLAUDE_SKILL_DIR}/assets/` (`template.html` for
   minimal, `template-full.html` for cards) into a user-approved output directory.
2. Load the matching `references/type-<name>.md` for layout conventions.
3. Replace the eyebrow, h1, and SVG body; preserve the source comment from §11.
4. Run the §9 taste gate.

### Validate a generated file

Run the bundled standard-library linter before delivery:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/lint_skin.py" "output/diagram.html"
```

When the project has a custom profile, pass it explicitly:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/lint_skin.py" \
  --style-guide ".diagram-design/style-guide.md" \
  "output/diagram.html"
```

---

## 11. Nexus delivery rules

- Deliver one HTML file with inline CSS and SVG. Google Fonts are optional enhancements; keep robust
  local fallback stacks so the diagram remains readable offline.
- Do not add JavaScript unless the user explicitly requests interaction. Static diagrams must remain
  printable and easy to embed.
- Every SVG needs a `viewBox`, `role="img"`, `<title>`, and `<desc>` that explains the structure in prose.
- Preserve this comment in HTML files derived from bundled templates:

  ```html
  <!-- Adapted from diagram-design by Cathryn Lavery, MIT; source commit a157f7616473d966d6f433cf0b4d4f1880603504. -->
  ```

- Open the result in a browser and inspect both desktop and narrow widths. Verify that labels do not
  collide, connectors stay independently traceable, and horizontal overflow is contained.
- Use a numerical chart workflow for evidence-backed statistics and `slide-maker` for complete presentations.
  This Skill owns standalone structural and conceptual diagrams.
