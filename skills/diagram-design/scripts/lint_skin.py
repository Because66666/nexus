#!/usr/bin/env python3
"""按当前视觉皮肤检查新增或修改过的图解 HTML。

2.0 之前的示例可能基于旧皮肤，使用 ``--all --baseline`` 跳过已记录的旧文件。
"""
...
def table_hexes(markdown, heading, style_guide):
    """返回指定标题后第一张 Markdown 表中的十六进制颜色。"""
...
def allowed_colors(style_guide):
    """读取当前样式指南允许的颜色集合。"""
    markdown = style_guide.read_text(encoding="utf-8")
    colors = table_hexes(markdown, "### Semantic roles", style_guide)
    colors.update(table_hexes(markdown, "### Series palette", style_guide))
    colors.update(table_hexes(markdown, "### Terminal skin", style_guide))
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=STYLE_GUIDE,
        help="style guide used to validate colors and fonts",
    )
...
def main():
    args = parse_args()
    colors, rgb_triplets = allowed_colors(args.style_guide)
