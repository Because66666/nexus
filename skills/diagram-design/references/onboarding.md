...
**Goal:** point the Skill at a design source — a website, an installed Skill, or a local folder — and
extract the palette + typography into the current project's `.diagram-design/style-guide.md`.

The built-in `references/style-guide.md` is an immutable default. Never edit it: Nexus shares platform
Skills across projects and users. Create the project-local directory only after the user approves the
proposed tokens.
...
[3] map to semantic roles (paper, ink, muted, accent, …)
      ↓
[4] propose a .diagram-design/style-guide.md diff
      ↓
...
Show the user what will change in `.diagram-design/style-guide.md`. Only the tokens table changes.
...
Write the approved tokens to `.diagram-design/style-guide.md`. Rebuild one diagram to verify the new
skin reads cleanly; do not rewrite the bundled examples.
...
Extract tokens from an installed Nexus, nxs, or Claude Code Skill that carries its own design system
(for example, a `brand-design` or `ui-kit` Skill).
...
1. `.agents/skills/<skill-name>/` (project or Nexus-compatible install)
2. `.claude/skills/<skill-name>/` (project Claude Code install)
3. `~/.agents/skills/<skill-name>/` or `~/.claude/skills/<skill-name>/` (user install)
4. Any path the user provides explicitly
...
Same as the URL method: run contrast checks, show the full diff against the current project-local
profile or built-in default, and write `.diagram-design/style-guide.md` only after the user approves.
...
The canonical active profile is `.diagram-design/style-guide.md`. If the user wants variants, keep
named files under `.diagram-design/profiles/` and copy the explicitly selected profile to the canonical
path before generating. Never infer or switch profiles silently.
