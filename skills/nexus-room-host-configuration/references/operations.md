...
The Room target is runtime-fixed: omit it for host operations. A host cannot call the owner-only `rooms.create` or `rooms.delete` operations from a Room. After `transfer_host` or removing the host, discard any outstanding plan and inspect again.
