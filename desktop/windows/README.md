...
- 窗口 chrome：WPF `WindowChrome` 用 60px 顶部玻璃框把 Web Header 延伸进非客户区并保留系统 caption controls，6px 外缘独占四边缩放命中；WebView2 的 `app-region` 只为 Header 空白处提供拖动、系统菜单和双击最大化，不自行重画窗口按钮，也不覆盖窗口缩放边界。
