...
//   - dispatch.go / review_dispatch.go / cancellation_dispatch.go: Room Assignment、Submission review-return 与 exact runtime cancellation outbox 的 list/claim/deliver/retry/cancel/recovery lease CAS；永久 Assignment Dispatch 失败只在 current active Plan/Spec 下同事务回收尚未启动的责任，stale graph 只终结旧 outbox；所有主动收束 live Attempt 的控制路径在同事务、状态更新前统一捕获 target。
//   - attempt.go / subagent_reconciliation.go: Attempt start/terminal 生命周期，以及精确 T+30s parent round exit grace deadline 的 durable schedule/跨进程 expired 查询。
//   - submission.go: immutable Submission、同事务 review-return outbox 与 append-only Acceptance。
