...
//   - execution.go / execution_transition.go / goal_retarget.go / evidence.go: Execution 创建、Goal revision graph supersede 与 reserved successor/predecessor 校验、无 active Plan 的已有 Execution 首次 Plan 原子写入、transient replacement/abandonment、只让 active Plan+Work Item 进入 WorkGraph 当前/历史选择的查询、Goal 绑定、runtime/scheduler 持久证据与完成。
