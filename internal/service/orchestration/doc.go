...
//   - context.go / execution_view.go / runtime_graph*.go / execution_alignment.go：面向当前 actor 的有界 <nexus_execution_context>、确定性 graph digest、current input/output、已验收依赖、terminal subagent result ref、恢复/guarded replan、Agent-selected objective alignment Gate，以及去除 capability identity、按 durable parent identity 保证子图边完整、保留 NodeRun 历史与 exact Tool Artifact、只记录已发生控制返回与 exact retry 事实的 Web WorkGraph/Runtime Graph 只读投影。
