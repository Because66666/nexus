...
//   - subagent_admission.go：SDK Agent tool 强准入、瞬时存储冲突退避重试、后端 launch binding、child Attempt 生命周期、parent round exit durable reconciliation 与 terminal evidence。
//   - review.go：Submission、Acceptance、依赖解锁与 completion audit。
//   - context.go / execution_view.go / runtime_graph*.go / execution_alignment.go：面向当前 actor 的有界 <nexus_execution_context>、确定性 graph digest、current input/output、已验收依赖、terminal subagent result ref、恢复/guarded replan、Agent-selected objective alignment Gate，以及去除 capability identity 的 Web WorkGraph/Runtime Graph 只读投影。
