...
//   - context.go / execution_view.go / runtime_graph*.go / execution_alignment.go：面向当前 actor 的有界 <nexus_execution_context>、确定性 graph digest、current input/output、已验收依赖、terminal subagent result ref、只反馈当前 Agent observed runtime facts 的恢复上下文、guarded replan、Agent-selected objective alignment Gate，以及去除 capability identity、保留每个 sibling Subagent、从结构化 Agent result 或受限历史 transcript 恢复 exact child Tool lifecycle、过滤无独立运行身份的 progress facet、按 exact launch tool 与 durable parent identity 保证子图归属和边完整、按用户可观察动作类别及运行/失败/控制边/Artifact/通用 visibility hint 提升 Tool 可见性、为每个 Subagent 保留有界代表 Tool 槽位并在关键动作后补入最近成功动作、以成功 submit_work 精确锚定 review/control return、避免重复领域 mutation 节点并保留 supporting inspection Tool 的 detail 历史与到达顺序无关 exact Tool Artifact、显式 partial/total、只记录已发生控制返回与 exact retry 事实的 Web WorkGraph/Runtime Graph 只读投影。
