...
//   - plan_document.go / plan_document_contract.go / plan_proposal.go / plan_materialization.go / plan_proposal_recovery.go：strict Nexus Plan Document v1 parser 与模型工具共用字段契约、按 Goal/document/current Execution 选择 canonical root boundary 的 durable non-authoritative sealed proposal、owner/session/scope/coordinator/target 以及 Goal owner/activation/objective/reserved successor/predecessor digest fence、ambient Goal 重新校验、authoritative command receipt 归因、不可继承的 initial lease 与 materializer CAS claim、receipt-proven blocked race 自动收敛、幂等原子 materialization，以及缺少 confirmer 时仍 fail closed 的 Goal confirmation pending saga 与跨 round/process reconciler。
...
//   - goal_policy.go / promotion.go / evidence.go / explicit_goal.go：只受权限与状态硬门槛约束的 Agent-selected adaptive promotion、runtime/scheduler 建议信号，以及含 canonical Goal objective activation、旧显式 Goal reservation 恢复的 explicit/adaptive Goal revision successor 双向 binding/confirmation。
