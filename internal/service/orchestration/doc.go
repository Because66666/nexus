...
//
// 成员清单：
//   - service.go / errors.go / work_binding.go / coordination_round.go：领域装配、输入校验、身份、structured Room WorkBinding/ReviewBinding、exact Goal 与 round-scoped Coordination capability fence、optimistic revision 错误。
...
//   - context.go / execution_view.go：面向当前 actor 的有界 <nexus_execution_context>、current input/output、已验收依赖、terminal subagent result ref、恢复/guarded replan、objective transition affordance，以及去除 capability identity 的 Web WorkGraph 只读投影。
