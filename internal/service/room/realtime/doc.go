...
//   - execution.go / execution_runtime.go / execution_context_usage.go / runtime_policy.go / recovery_context.go / execution_slot_status.go / interrupt.go / subagent_idle_drain.go：slot 执行主链、runtime 选项、每 Agent 终态上下文占用快照及 Session 元数据持久化、上一轮失败恢复上下文、Room 工具权限策略、连接诊断、终态同步、中断控制值与展示文案隔离、父子 usage 后台重试。
