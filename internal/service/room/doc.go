...
//
// 文件按职责前缀分组：
//   - service.go / crud.go / conversation_crud.go / member.go / query.go：Room 服务装配、房间、带 configuration_version CAS 与 authority epoch 的持久成员参与闸门，以及至少保留一条的 conversation 数据操作；每个 Room 最多保留一个显式未开始 draft。
