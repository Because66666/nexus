...
//     回滚与 objective revision adoption，并支持按 owner 强制回收；key 级启动与关闭栅栏、
//     client 换代和 lease 条件关闭也在此收口。interrupt.go 额外区分 sole-running-round
//     provider interrupt 与 exact local context cancellation，并在 provider interrupt 窗口阻止
//     successor admission；shared session 不回退为可能误伤 successor 的 interrupt。
