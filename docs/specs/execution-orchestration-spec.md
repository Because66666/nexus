...
4. **一次提交完整意图。** `plan_execution` 批量提交完整 WorkGraph；服务端 mint opaque IDs，并返回 logical key → ID 映射、Ready 项和下一动作。禁止让模型用多个低级 create/update 调用拼半成品 Plan。
...
6. **工具结果必须可恢复。** 每个 mutation 返回 `applied | no_op | rejected`、稳定 reason code、最新 snapshot revision、实际改变和有序 `next_actions`。错误必须说明怎样修复，例如先验收哪个 dependency、由哪个 owner 提交、或先刷新哪个 Plan。
