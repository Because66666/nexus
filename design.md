...
- Agent 详情等页面栏目使用纯文字导航，并按身份、技能、记忆、工具、联络排序；未选中项降为中灰，当前项使用 32px 低对比中性轻底确认。联系人侧栏已提供当前 Agent 身份时，详情 Header 不重复头像与名称。栏目语义已经明确时不再叠加图标或下划线。Agent 技能页直接从已启用与可启用分组开始，不重复展示总量摘要或常驻手动刷新；Skill 使用紧凑分隔列表，分组已表达的状态不在行内重复。可启用搜索在桌面与分组标题同排、窄屏换行，只有筛选时才显示命中数。技能来源等只改变当前结果集的模式属于筛选，与搜索并列放在内容工具行，使用同样克制的文字选中态，不伪装成页面导航。
- 能力目录以正文中的标题与单句说明作为唯一页面身份，不再在 Surface Header 重复“图标 + 栏目名”；导入、刷新、创建等页面级动作与正文标题同排，窄屏自然换到标题下方。标准目录、设置与联系人共用铺满工作面的管理内容区和同一 Header 几何；Web 中标题与动作必须和侧栏品牌栏、macOS 中必须和原生窗口控件共享垂直中心节奏。普通目录在桌面显示三列；定时任务保留四列关系并在窄窗内横向滚动。
