...
- 能力目录以正文中的标题与单句说明作为唯一页面身份，不再在 Surface Header 重复“图标 + 栏目名”；导入、刷新、创建等页面级动作与正文标题同排，窄屏自然换到标题下方。标准目录、设置与联系人共用铺满工作面的管理内容区和同一 Header 几何，普通目录在桌面显示三列；定时任务保留四列关系并在窄窗内横向滚动。
