...
- Agent 详情等页面栏目使用纯文字导航，并按身份、技能、记忆、工具、联络排序；未选中项降为中灰，当前项使用 32px 低对比中性轻底确认。联系人侧栏已提供当前 Agent 身份时，详情 Header 不重复头像与名称。栏目语义已经明确时不再叠加图标或下划线。Agent 技能页直接从已启用与可启用分组开始，不重复展示总量摘要或常驻手动刷新；Skill 使用紧凑分隔列表，分组已表达的状态不在行内重复。技能来源等只改变当前结果集的模式属于筛选，与搜索并列放在内容工具行，使用同样克制的文字选中态，不伪装成页面导航。
