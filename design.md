...
- 层级有序：背景、内容面、输入控件、按钮与浮层必须在静止状态即可辨认；使用清晰发丝边界、轻填充、短阴影和内高光建立层级，禁止厚玻璃与大面积重阴影。
...
Token 保持四层映射；命名和依赖方向稳定，主题值按可读性与层级辨识度统一校准：
...
- `material-*` 中的渐变属于 image 值，语义 token 必须直接引用，不能放进只接受 color 的 `color-mix()`；需要调节透明度时应在材质源头提供合法变体。
...
| 配方 | 规则 |
| --- | --- |
| `desktop-rail` | 透明透出页面纹理，以明确但安静的边界和内侧分隔标记导航区 |
| `surface-panel` | 单层填充、清晰发丝边界；只用短阴影与环境底分离 |
| `surface-inset` | 低对比底、细边界和浅内阴影，表达输入或嵌入关系，不悬浮 |
| `surface-card` | 实体轻填充、可见边界和短外阴影，表达独立内容或可点击区域 |
| `surface-popover` | 更高不透明度、清晰边界和更高一档阴影 |
| `chip-default` | 可见边界、低对比填充和轻内高光 |
| `input-shell` | 静止态已有底色、边界和浅内凹感；hover / focus 再增强边界与 ring |

- 卡片必须有内容或交互职责；当前项用低透明度 primary 背景或边界，不靠粗体堆叠。
- 阴影只承担层级识别：输入框向内、卡片与按钮短距离向外、弹窗最高；不得给正文行、普通列表项或整块工作区内容逐项加阴影。
- 按钮在静止态就要与背景分离；主按钮的色彩和阴影高于次按钮，ghost / text 按钮保持平面。
...
1. 页面能否按“背景 → rail → plane → 控件 → 浮层”读懂，且不依赖 hover 才识别可点击区域？
2. 正文是否强于状态和装饰？字体、字重、列表与代码密度是否一致？
3. 是否使用语义 token，而非复制 raw color、渐变或阴影？
4. `light` / `dark` / `rain`、窄屏、键盘焦点和减少动效是否仍可用？

如果删除一个渐变、圆角、标签或动画后，用户仍能更快判断位置、状态和下一步，默认删除它；如果删除边界或短阴影会让控件与容器在静止态混成一层，则保留层级线索。
