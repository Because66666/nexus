...
- 侧栏折叠后，展开开关必须脱离 52px 裁剪壳；Web 中与 48px Dock 共用水平中心轴，macOS 中固定在窗口控制按钮右侧并与其共用垂直中心轴。侧栏外缘竖线从 Header 底部开始，禁止穿入系统按钮区域；正文保持原有内容轴，只有位于原生标题栏平面的共享 Header 增加横向安全区，位置变化不应引起 Dock 或正文纵向跳动。
...
- Agent 详情等页面栏目使用纯文字导航，并按身份、技能、记忆、工具、联络排序；未选中项降为中灰，当前项使用 32px 低对比中性轻底确认。联系人侧栏已提供当前 Agent 身份时，详情 Header 不重复头像与名称。栏目语义已经明确时不再叠加图标或下划线。Agent 技能页直接从已启用与可启用分组开始，不重复展示总量摘要或常驻手动刷新；可启用搜索在桌面与分组标题同排、窄屏换行，只有筛选时才显示命中数。技能卡片使用紧凑的三列、两列、单列响应式网格，并保留两行说明；能力页已安装、更新与社区结果共用头像驱动目录卡，首行放身份、名称、状态和动作，说明占两行，来源与使用信息固定在末行。能力库与 Agent 卡片共用由稳定 Skill 名称生成的 rounded-square 身份标记，外轮廓跟随系统 Agent 头像，所有身份面只调用公共头像组件；静态 SVG 优先选用旋转对称的闭合曲线，种子只决定曲线族、阶数、细节和整体朝向，数学原点始终映射到头像中心，同一 Skill 在目录、详情和预览中不得变化。分组与开关已表达的启用状态不在行内重复，系统内置状态只由锁定徽标表达。Nexus 随产品提供的 Skill 在目录、详情、Agent、Room 和 Composer 中共用按界面语言切换的展示说明，不改动真实 Skill 元数据、指令或同名用户 Skill。技能来源等只改变当前结果集的模式属于筛选，与搜索并列放在内容工具行，使用同样克制的文字选中态，不伪装成页面导航。
- 能力目录以正文中的标题与单句说明作为唯一页面身份，不再在 Surface Header 重复“图标 + 栏目名”；导入、刷新、创建等页面级动作与正文标题同排，窄屏自然换到标题下方。标准目录、设置与联系人共用铺满工作面的管理内容区和同一 Header 几何；Web 中标题与动作必须和侧栏品牌栏、macOS 中必须和原生窗口控件共享垂直中心节奏。普通目录在桌面显示三列，并以一致的细边框和方形身份图标建立条目层级；定时任务保留四列关系并在窄窗内横向滚动。
